...
void OnDataRecv(const uint8_t * mac, const uint8_t *incomingData, int len) {                                  // -------- réception données extérieur --------------
  ...
  myGLCD.setTextColor(TFT_YELLOW,TFT_BLACK); myGLCD.drawFloat(t_max, 1, 10, 330, 6); myGLCD.setTextColor(TFT_YELLOW,TFT_BLACK); myGLCD.drawFloat(t_min, 1, 10, 400, 6);  //affiche mini maxi

/*if ( (millis() - temps_moy) > 1000*60) {
      // affichage de la moyenne température exterieur
